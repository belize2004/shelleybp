...
interface PageClientProps {
  isMobile: boolean
  pensacolaFamilyPhotography?: any
}

export default function PageClient({isMobile, pensacolaFamilyPhotography = null}: PageClientProps) {
  ...
  return (
    <>
      ...
      {isMobile ? (
        ...
        >
          {pensacolaFamilyPhotography?.photos?.map((image, idx) => {
            const onClick = () => {
      ...
      <div className="flex flex-col p-8 gap-8">
        {pensacolaFamilyPhotography?.blogs?.map((b) => <BlogCard key={b._id} blogPost={b} />)}
      </div>
    </>
  )
}
